...
class RNN_ENCODER(nn.Module):
    ...
    def define_module(self):
        ...
        if self.rnn_type == 'LSTM':
            ...
            def bert_caption_preprocess(txt_batch):
                return ["[CLS] " + txt + " [SEP]" for txt in txt_batch]
            
            self.encoder = bert_caption_preprocess
            self.drop = lambda x: x
            self.rnn = BertTokenizer.from_pretrained('bert-base-uncased')
            # https://mccormickml.com/2019/05/14/BERT-word-embeddings-tutorial/
            raise NotImplementedError("Text Encoder type:'%s' is not implemented" % self.rnn_type)
        else:
            raise NotImplementedError("Text Encoder type:'%s' is not implemented" % self.rnn_type)
